...
router.get("/", async (req,res) => {
    ...
    try{
      //populate必須在exec前面，因為exec如果在前面已經執行了查詢並返回一個promise，這時查詢已完成，不能在已完成的查詢結果上應用.populate()
...
router.get("/instructor/:_instructor_id", async (req , res) => {
    let {_instructor_id } = req.params;
     let coursesFound = await Course.find( { instructor: _instructor_id}).populate("instructor", [
        "username",
        "email",
    ]) .exec();
...
router.post("/", async (req,res) => {
    ...
    if(req.user.isStudent ()){
        return res.status(400).send("只有講師才能發佈新課程，若您已經是講師，請透過講師帳號登入。");
    };
